...
Tanken4:
  SetLocal(1,Sum(Local(1),+7000));
  if(GreaterThan(Local(1),30000)) return(Call("FullTank"));
  Message("Treibstoff: %d von 30000",this(),Local(1));
 return(1);
FullTank:
  SetLocal(1,30000);
...
ContainedDigSingle:
  ...
  if(LessThan(Local(1),800)) AddMenuItem("Treibstoff?","Display",8P2A,Par(0));
  if(GreaterThan(Local(1),799)) AddMenuItem("Treibstoff?","Display",8P2M,Par(0));
  AddMenuItem("Tuningstufen?","TuningInfo",MTP3,Par(0));
  AddMenuItem("Licht","Lichtschalten",MENL,Par(0));
 return(1);
Display:
  Message("Restbenzin (Diesel): %d von 30000",this(),Local(1));
  return(1);
...
public GetResearchBase: return(C18T);
