...
Tanken2:
  SetLocal(1,Sum(Local(1),+5500));
  if(GreaterThan(Local(1),20000)) return(Call("FullTank"));
  Message("Treibstoff: %d von 20000",this(),Local(1));
 return(1);
FullTank:
  SetLocal(1,20000);
...
ContainedDigSingle:
  ...
  if(LessThan(Local(1),200)) AddMenuItem("Treibstoff?","Display",8P2A,Par(0));
  if(GreaterThan(Local(1),199)) AddMenuItem("Treibstoff?","Display",8P2M,Par(0));
  AddMenuItem("Tuningstufen?","TuningInfo",MTP3,Par(0));
  AddMenuItem("Licht","Lichtschalten",MENL,Par(0));
 return(1);
Display:
  Message("Restbenzin (Super): %d von 20000",this(),Local(1));
  return(1);
...
public GetResearchBase: return(OA14);
